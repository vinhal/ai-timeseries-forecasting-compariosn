...
def exec(
    path='2_bi-lstm_bono_multivariate_one-step_regression',
    epochs=100,
    batch_size=64,
    learning_rate=0.001,
    variant=1,
    ...
    # CODE
    file_name = '../../_data_/bono/bono.csv'
    timestamp_col = 'Timestamp'
    value_col = 'net_packets'
    sep = ';'

    lag = 120  # 168 hours = 1 week
    ...
    # data.insert(0, timestamp_col, timestamp)
    ...
    def compare():
        ...
        for item in x_val:
            tic = timeit.default_timer()
            y_pred.append(model.predict(np.array([item]))[0][0])
            toc = timeit.default_timer()
...
for i in range(1, 11):
    exec(
        path=f'2_bi-lstm_bono_multivariate_one-step_regression_{i}',
        epochs=200,
        batch_size=64,
        learning_rate=None,
        variant=1
