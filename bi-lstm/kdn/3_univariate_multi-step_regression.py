...
def exec(
    path='3_bi-lstm_kdn_univariate_multi-step_regression',
    epochs=100,
    batch_size=64,
    learning_rate=0.001,
    variant=1
    ...
    ger = pd.DataFrame()
    ger['target'] = y_val.reshape(y_val.shape[0] * y_val.shape[1])
    ...
    model.add(keras.layers.Dense(y_tr.shape[1], activation='linear'))
...
for i in range(1, 11):
    exec(
        path=f'3_bi-lstm_kdn_univariate_multi-step_regression_{i}',
        epochs=200,
        batch_size=16,
        learning_rate=0.01,
        variant=2
