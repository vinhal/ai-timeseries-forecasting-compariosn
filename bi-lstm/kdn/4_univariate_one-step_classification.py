...
def exec(
    path='4_bi-lstm_kdn_univariate_one-step_classification',
    epochs=100,
    batch_size=64,
    learning_rate=0.001,
    variant=1
    ...
    ger = pd.DataFrame()
    ger['target'] = y_val
    ger.to_csv('out.csv', index=False)
    ...
    if variant == 1:
        model.add(keras.layers.Bidirectional(keras.layers.LSTM(
            128, return_sequences=True), input_shape=(x_tr.shape[1], x_tr.shape[2])))
        model.add(keras.layers.Bidirectional(keras.layers.LSTM(64)))
    if variant == 2:
        model.add(keras.layers.Bidirectional(keras.layers.LSTM(
            256, return_sequences=True), input_shape=(x_tr.shape[1], x_tr.shape[2])))
        model.add(keras.layers.Bidirectional(keras.layers.LSTM(128)))
    if variant == 3:
        model.add(keras.layers.Bidirectional(keras.layers.LSTM(
            128), input_shape=(x_tr.shape[1], x_tr.shape[2])))
...
for i in range(1, 11):
    exec(
        path=f'4_bi-lstm_kdn_univariate_one-step_classification_{i}',
        epochs=100,
        batch_size=64,
        learning_rate=0.001,
        variant=1
