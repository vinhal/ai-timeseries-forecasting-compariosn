...
def exec(
    path='1_cnn-lstm_bono_univariate_one-step_regression',
    epochs=100,
    batch_size=64,
    learning_rate=0.001,
    variant=1,
    ...
    def compare():
        ...
        for item in x_val:
            tic = timeit.default_timer()
            y_pred.append(model.predict(
                item.reshape(1, -1, 1))[0][0])
            toc = timeit.default_timer()
...
for i in range(1, 11):
    exec(
        path=f'1_cnn-lstm_bono_univariate_one-step_regression_{i}',
        epochs=300,
        batch_size=64,
        learning_rate=0.01,
        variant=3
