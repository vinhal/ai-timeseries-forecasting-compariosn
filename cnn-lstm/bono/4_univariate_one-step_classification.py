...
def exec(
    path='4_cnn-lstm_bono_univariate_one-step_classification',
    epochs=100,
    batch_size=64,
    learning_rate=0.001,
    variant=1
    ...
    if variant == 1:
        ...
        cnn.add(keras.layers.Conv1D(64, 3, padding='same',
                                    activation='tanh', input_shape=(x_tr.shape[1], x_tr.shape[2])))
        cnn.add(keras.layers.Conv1D(32, 5, padding='same',
                                    activation='tanh', input_shape=(x_tr.shape[1], x_tr.shape[2])))
        cnn.add(keras.layers.Flatten())

        model.add(keras.layers.TimeDistributed(
            cnn, input_shape=(x_tr.shape[1], x_tr.shape[2])))
        # model.add(keras.layers.RepeatVector(4))
        model.add(keras.layers.LSTM(128))
    if variant == 2:
        cnn = keras.Sequential()
        cnn.add(keras.layers.Conv1D(128, 3, padding='same',
                                    activation='tanh', input_shape=(x_tr.shape[1], x_tr.shape[2])))
        cnn.add(keras.layers.Conv1D(64, 5, padding='same',
                                    activation='tanh', input_shape=(x_tr.shape[1], x_tr.shape[2])))
        cnn.add(keras.layers.Flatten())
    ...
    model.add(keras.layers.Dense(label_count, activation='softmax'))

    model.summary()
    model.compile(loss='sparse_categorical_crossentropy', optimizer=keras.optimizers.Adam(learning_rate=learning_rate) if learning_rate != None else 'adam',
                  metrics=['acc'])
    ...
    def compare():
        ...
        for item in x_val:
            tic = timeit.default_timer()
            y_pred.append(get_value(model.predict(
                item.reshape(1, -1, 1))[0]))
            toc = timeit.default_timer()
...
for i in range(1, 11):
    exec(
        path=f'4_cnn-lstm_bono_univariate_one-step_classification_{i}',
        epochs=100,
        batch_size=64,
        learning_rate=0.001,
        variant=3
