...
def exec(
    path='4_enc-dec-cnn-lstm_bono_univariate_one-step_classification',
    epochs=100,
    batch_size=64,
    learning_rate=0.001,
    variant=1
    ...
    def split_data(seq, num, future):
        ...
        for i in range(0, (len(seq)-num), 1):
            input_ = seq[i:i+num]
            output = [get_class(seq[i+num]) for t in range(0, future)]
            x.append(input_)
            y.append(output)

        return np.array(x), np.array(y)

    x, y = split_data(values, lag, future_steps)  # X values, Y targets
    ...
    def compare():
        ...
        for item in x_val:
            tic = timeit.default_timer()
            y_pred.append([get_value(t) for t in model.predict(
        ...
        print('y_true LEN', np.array(y_true).shape)
        print('y_pred LEN', np.array(y_pred).shape)
...
for i in range(1, 11):
    exec(
        path=f'4_enc-dec-cnn-lstm_bono_univariate_one-step_classification_{i}',
        epochs=100,
        batch_size=16,
        learning_rate=0.001,
        variant=2
