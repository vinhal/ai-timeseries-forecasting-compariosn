...
def exec(
    path='4_gru_bono_univariate_one-step_classification',
    epochs=100,
    batch_size=64,
    learning_rate=0.001,
    variant=1
    ...
    if variant == 1:
        model.add(keras.layers.GRU(128, input_shape=(
            x_tr.shape[1], x_tr.shape[2]), return_sequences=True))
        model.add(keras.layers.GRU(64))
    if variant == 2:
        model.add(keras.layers.GRU(256, input_shape=(
            x_tr.shape[1], x_tr.shape[2]), return_sequences=True))
...
for i in range(1, 11):
    exec(
        path=f'4_gru_bono_univariate_one-step_classification__{i}',
        epochs=100,
        batch_size=64,
        learning_rate=0.01,
        variant=2
