...
def exec(
    path='3_svr_bono_univariate_multi-step_regression',
    kernel='rbf',
    epsilon=0.05,
    gamma=1,
    C=0.1,
    ...
    model = MultiOutputRegressor(
        SVR(kernel=kernel, gamma=gamma, C=C, epsilon=epsilon, verbose=1))
...
for i in range(1, 11):
    exec(
        path=f'3_svr_bono_univariate_multi-step_regression_{i}',
        kernel='rbf',
        epsilon=0.05,
        gamma=0.001,
        C=100,
